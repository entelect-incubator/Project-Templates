			...
			if (_data) {
				...
				if (_data["ValidationErrors"]) {
					...
					for (let key in _data["ValidationErrors"]) {
						if (_data["ValidationErrors"].hasOwnProperty(key)) (this.validationErrors as any)![key] = _data["ValidationErrors"][key] !== undefined ? _data["ValidationErrors"][key] : [];
					}
				}
		...
		toJSON(data?: any) {
			...
			if (this.validationErrors) {
				...
				for (let key in this.validationErrors) {
					if (this.validationErrors.hasOwnProperty(key)) (data["ValidationErrors"] as any)[key] = (this.validationErrors as any)[key];
				}
			}
		...
		override init(_data?: any) {
			...
			if (_data) {
				if (Array.isArray(_data["Data"])) {
					this.data = [] as any;
					for (let item of _data["Data"]) this.data!.push(PizzaModel.fromJS(item));
				}
				this.count = _data["Count"];
			}
		}
